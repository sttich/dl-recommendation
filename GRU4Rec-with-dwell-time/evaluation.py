...
def evaluate_sessions_batch(pr, test_data, items=None, cut_off=20, batch_size=100, mode='standard',
                            session_key='SessionId', item_key='ItemId',
                            time_key='Time',dwell_time=75):  # mode='standard' 'conservative' 'median' 'tiebreaking'
    '''
    ...
    if dwell_time != 0:  # item boosting: the number of occurrence is dwell_time_i / threshold_time
        print("calculating item boosting...")
        is_eval=[]
        boosting_count = 0
        data_t = pd.DataFrame(pd.np.empty((3 * len(test_data), len(test_data.columns))) * pd.np.nan, columns=test_data.columns)
        data_values = test_data.values  # use numpy to accelerate
        data_t_values = data_t.values
        for i in range(0, (len(data_values) - 1)):
            boosting_times = 1
            line1 = test_data.iloc[i]
            line2 = test_data.iloc[i + 1]
            if line2[session_key] == line1[session_key]:
                boosting_times = int(
                    (line2[time_key] - line1[time_key]) / dwell_time) + 1
            for j in range(0, boosting_times):
                data_t_values[boosting_count] = data_values[i]
                boosting_count += 1
        data_t_values[boosting_count] = data_values[(len(data_values) - 1)]
        data_t.loc[0] = test_data.loc[0]
        test_data = data_t[:boosting_count + 1]
        print("item boosting completed, boosting_count: {}, data length: {}".format(boosting_count, len(test_data)))
    ...
    while True:
        ...
        for i in range(minlen - 1):
            ...
            if sampled_items:
                ...
                if mode == 'standard':
                    ranks = (preds.values.T[valid_mask].T > np.diag(preds.loc[in_idx].values)[valid_mask]).sum(
                        axis=0) + 1
                elif mode == 'conservative':
                    ranks = (preds.values.T[valid_mask].T >= np.diag(preds.loc[in_idx].values)[valid_mask]).sum(axis=0)
                elif mode == 'median':
    ...
    return recall / evalutation_point_count, mrr / evalutation_point_count, ndcg/evalutation_point_count


def evaluate_gpu(gru, test_data, items=None, session_key='SessionId', item_key='ItemId', time_key='Time', cut_off=20,
                 batch_size=100, mode='conservative', dwell_time=0):
    ...
    # ranks is an array of length batch_size.
    if mode == 'standard':
        ranks = (others > targets).sum(axis=0) + 1
    elif mode == 'conservative':
        ranks = (others >= targets).sum(axis=0)
    ...
    REC = (ranks <= cut_off).sum()  # the number of relevant and retrieved item
    ...
    if dwell_time != 0:  # item boosting: the number of occurrence is dwell_time_i / threshold_time
        ...
        print("item boosting completed, boosting_count: {}, data length: {}".format(boosting_count, len(data)))

    test_data_items = test_data.ItemIdx.values
    if items is not None:
        item_idxs = gru.itemidmap[items]
    recall, mrr, relevant, retrieved, average_precision, normalized_dcg = 0, 0, 0, 0, 0, 0
    iters = np.arange(batch_size)
    maxiter = iters.max()
    offset_sessions = np.zeros(test_data[session_key].nunique() + 1, dtype=np.int32)  # offset of each session
    offset_sessions[1:] = test_data.groupby(session_key).size().cumsum()
    start = offset_sessions[iters]  # start index of each session in this batch
    end = offset_sessions[iters + 1]  # end index of each session in this batch
    ...
    while not finished:
        ...
        for i in range(minlen - 1):
            ...
            if items is not None:
                ...
                y = out_idx
            rec_batch, mrr_batch = evaluate(in_idx, y, len(iters), *cidxs)  # returned by theano.function
            recall += rec_batch
            mrr += mrr_batch
            relevant += len(iters)
            retrieved += len(iters) * cut_off
    ...
    recall_res = recall / relevant
    mrr_res = mrr / relevant
    precision_res = recall / retrieved
    if precision_res == 0 and recall_res == 0:
        f1_score_res = np.nan
    else:
        f1_score_res = 2 * (precision_res * recall_res) / (precision_res + recall_res)
    return recall_res, mrr_res, precision_res, f1_score_res
