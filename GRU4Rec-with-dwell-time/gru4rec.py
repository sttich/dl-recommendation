...
"""
import sys
#sys.path.append('/nfsshare/home/zhangdanning/lib/python3.6/site-packages')
sys.path.append('/clusterdata/home/zhangdanning/lib/python3.7/site-packages')
import theano
...
class GRU4Rec:
    ...
    dwell_time : int
        the threshold time for item boosting,
        cite: Incorporating dwell time in session-based recommendations with recurrent Neural networks
    """
    ...
    def __init__(self, loss='bpr-max', final_act='linear', hidden_act='tanh', layers=[100],
                 ...
                 adapt='adagrad', adapt_params=[], grad_cap=0.0, bpreg=1.0,
                 sigma=0.0, init_as_normal=False, train_random_order=False, time_sort=True,
                 session_key='SessionId', item_key='ItemId', time_key='Time', dwell_time=75):
        self.layers = layers
    ...
    def init(self, data):
        ...
        if self.dwell_time != 0:  # item boosting: the number of occurrence is dwell_time_i / threshold_time
            print("calculating item boosting...")
            boosting_count = 0
            data_t = pd.DataFrame(pd.np.empty((3 * len(data), len(data.columns))) * pd.np.nan, columns=data.columns)
    ...
    def fit(self, data, sample_store=10000000):
        ...
        offset_sessions, data = self.init(data)
        if self.n_sample:
