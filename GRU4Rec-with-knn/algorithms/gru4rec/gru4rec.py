...
from algorithms.gpu.gpu_ops import gpu_diag_wide
...
class GRU4Rec:
    ...
    '''
    def __init__(self, layers, n_epochs=10, batch_size=32, dropout_p_hidden=0.0, learning_rate=0.2, momentum=0.3, adapt='adagrad', decay=0.9, grad_cap=0, sigma=0,
                 init_as_normal=False, reset_after_session=True, loss='bpr-max', hidden_act='tanh', final_act='elu-0.5', train_random_order=False, lmbd=0.0,
                 session_key='SessionId', item_key='ItemId', time_key='Time', n_sample=2048, sample_alpha=0.75,bpreg=1):
        self.layers = layers
    ...
    def set_loss_function(self, loss):
        if loss == 'cross-entropy':
            ...
            self.loss_function = self.bprmax
        elif loss == 'top1':
            self.loss_function = self.top1
        elif loss == 'top1-max':
            self.loss_function = self.top1max
        elif loss == 'xe_logit':
