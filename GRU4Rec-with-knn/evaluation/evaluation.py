...
import numpy as np
import pandas as pd
#from evaluation import ranking_evaluator

def evaluate_sessions(pr, metrics, test_data, train_data, items=None, session_key='SessionId', item_key='ItemId', time_key='Time',mode='conservative'):
    '''
    ...
    for i in range(len(test_data)):
        ...
        in_time=test_data[time_key].values[i]
        if prev_sid != sid:
            prev_sid = sid
        else:
...
#             preds += 1e-8 * np.random.rand(len(preds)) #Breaking up ties\
            ...
            ###############################################################################################################################
            test_data_out=test_data[test_data[session_key]==sid]
            ground_truth_list=list(test_data_out[test_data_out[time_key]>=in_time].sort_values(by='Time',ascending=True)[item_key])

            for m in metrics:
                m.add( preds, iid,ground_truth_list)
            ########################################################################################################################################
        prev_iid = iid
...
def evaluate_sessions_batch(pr, test_data, train_data, items=None, cut_off=20, session_key='SessionId', item_key='ItemId', time_key='Time', batch_size=10, break_ties=True,mode='conservative' ):
    '''
