...
def evaluate_sessions(pr, metrics, test_data, train_data, buy_data, items=None, cut_off=20, session_key='SessionId', item_key='ItemId', time_key='Time', type_key='Type'): 
    '''
    Evaluates only the buy events from buy_data per session for the algorithms wrt. the given metrics. Has no batch evaluation capabilities. 
    ...
    for i in range(len(test_data)):
        ...
        if prev_sid != sid:
            ...
            if( test_data['Type'].values[i] == 1 ): #buy
