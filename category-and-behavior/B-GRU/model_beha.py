...
class GRU4Rec:
    ...
    def __init__(self, sess, args):
        ...
        self.predict_state_beha = [np.zeros([self.batch_size, self.behavior_size], dtype=np.float32) for _ in
                                   range(self.layers)]
        ckpt = tf.train.get_checkpoint_state(self.checkpoint_dir)
        if ckpt and ckpt.model_checkpoint_path:
            self.saver.restore(sess, '{}/gru-model_heha-{}'.format(self.checkpoint_dir, args.test_model))
    ...
    def build_model(self):
        ...
        self.state_item = [tf.placeholder(tf.float32, [self.batch_size, self.item_size], name='rnn_state') for _ in
                      range(self.layers)]
        self.state_beha = [tf.placeholder(tf.float32, [self.batch_size, self.behavior_size], name='rnn_state') for _ in
                      range(self.layers)]
        ...
        with tf.variable_scope('gru_layer_beha'):
            ...
            embedding_behavior = tf.get_variable('embedding_behavior', [self.n_behaviors, self.behavior_size],
                                                 initializer=initializer)
            inputs_beha = tf.nn.embedding_lookup(embedding_behavior, self.Behavior)

        with tf.variable_scope('output'):
            output=tf.concat([output_item,inputs_beha],axis=1)
            output=tf.layers.dense(output,self.latent_size,activation='tanh')
            softmax_W = tf.get_variable('softmax_w', [self.n_items, self.latent_size], initializer=initializer)
    ...
    def fit(self, data,sample_store=10000000):
        ...
        for epoch in range(self.n_epochs):
            ...
            while not finished:
                ...
                out_beha = data.action_type.values[start]
                for i in range(minlen - 1):
                    in_idx = out_idx
                    in_beha = out_beha
                    out_idx = data.ItemIdx.values[start + i + 1]
                    out_beha = data.action_type.values[start + i + 1]
                    ...
                    fetches = [self.cost, self.final_state_item, self.global_step, self.lr,
                               self.train_op]
                    feed_dict = {self.X: in_idx, self.Y: y, self.Behavior: in_beha}
            ...
            self.saver.save(self.sess, '{}/gru-model_heha'.format(self.checkpoint_dir), global_step=epoch)

    def predict_next_batch(self, session_ids, input_item_ids, input_behavior, itemidmap, batch=50):
        '''
        ...
        feed_dict = {self.X: in_idxs, self.Behavior: input_behavior}
        for i in range(self.layers):
            feed_dict[self.state_item[i]] = self.predict_state_item[i]
        preds, self.predict_state_item = self.sess.run(fetches, feed_dict)
