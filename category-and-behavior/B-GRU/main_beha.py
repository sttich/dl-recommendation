...
if __name__ == '__main__':
    ...
    item_dic = {}
    itemids = list(data.ItemId.unique())
    for i, j in enumerate(itemids):
        item_dic[j] = i
    data['ItemId'] = data['ItemId'].apply(lambda x: item_dic[x])
    valid['ItemId'] = valid['ItemId'].apply(lambda x: item_dic[x])

    session_dic = {}
    sessionids = list(data.SessionId.unique()) + list(valid.SessionId.unique())
    sessionids = list(set(sessionids))
    for i, j in enumerate(sessionids):
        session_dic[j] = i
    data['SessionId'] = data['SessionId'].apply(lambda x: session_dic[x])
    valid['SessionId'] = valid['SessionId'].apply(lambda x: session_dic[x])
    ...
    with tf.Session(config=gpu_config) as sess:
        gru = model_beha.GRU4Rec(sess, args)
        if args.is_training:
            gru.fit(data)
        else:
            print(args.test_model)
            res = evaluation_beha.evaluate_sessions_batch(gru, data, valid, cut_off=5, batch_size=args.batch_size)
            print('Recall@5: {}'.format(res[0]))
            print('MRR@5: {}'.format(res[1]))
            print('NDCG@5: {}'.format(res[2]))

            res = evaluation_beha.evaluate_sessions_batch(gru, data, valid, cut_off=10, batch_size=args.batch_size)
            print('Recall@10: {}'.format(res[0]))
            print('MRR@10: {}'.format(res[1]))
            print('NDCG@10: {}'.format(res[2]))

            res = evaluation_beha.evaluate_sessions_batch(gru, data, valid, cut_off=20, batch_size=args.batch_size)
