...
if __name__ == '__main__':
    ...
    with tf.Session(config=gpu_config) as sess:
        gru = model_con.GRU4Rec(sess, args)
        if args.is_training:
            gru.fit(data)
        else:
            print(args.test_model)
            res = evaluation_con.evaluate_sessions_batch(gru, data, valid, cut_off=5, batch_size=args.batch_size)
            print('Recall@5: {}'.format(res[0]))
            print('MRR@5: {}'.format(res[1]))
            print('NDCG@5: {}'.format(res[2]))

            res = evaluation_con.evaluate_sessions_batch(gru, data, valid, cut_off=10, batch_size=args.batch_size)
            print('Recall@10: {}'.format(res[0]))
            print('MRR@10: {}'.format(res[1]))
            print('NDCG@10: {}'.format(res[2]))

            res = evaluation_con.evaluate_sessions_batch(gru, data, valid, cut_off=20, batch_size=args.batch_size)
