...
def evaluate_sessions_batch(model, train_data, test_data, cut_off=20, batch_size=50, session_key='SessionId', item_key='ItemId', time_key='Time'):
    ...
    in_cate= np.zeros(batch_size, dtype=np.int32)
    np.random.seed(42)
    while True:
        valid_mask = iters >= 0
        if valid_mask.sum() == 0:
        ...
        in_cate[valid_mask]=test_data['category_id'].values[start_valid]

        for i in range(minlen-1):
            out_idx = test_data[item_key].values[start_valid+i+1]
            out_cate=test_data['category_id'].values[start_valid+i+1]
            preds = model.predict_next_batch(iters, in_idx,in_cate,itemidmap,batch_size)
            #preds = model.predict_next_batch(iters, in_idx, itemidmap, batch_size)
            preds.fillna(0, inplace=True)
            in_idx[valid_mask] = out_idx
            in_cate[valid_mask]=out_cate
