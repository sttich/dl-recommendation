...
class GRU4Rec:
    ...
    def __init__(self, sess, args):
        ...
        self.predict_state = [np.zeros([self.batch_size, self.item_size+self.category_size], dtype=np.float32) for _ in range(self.layers)]
        ckpt = tf.train.get_checkpoint_state(self.checkpoint_dir)
        if ckpt and ckpt.model_checkpoint_path:
            self.saver.restore(sess, '{}/gru-model_rsc15_con_100_20-{}'.format(self.checkpoint_dir, args.test_model))
    ...
    def build_model(self):
        ...
        self.state = [tf.placeholder(tf.float32, [self.batch_size, self.item_size+self.category_size], name='rnn_state') for _ in
                      range(self.layers)]
        ...
        with tf.variable_scope('gru_layer_item'):
            ...
            cell = rnn_cell.GRUCell(self.item_size+self.category_size, activation=self.hidden_act)
            ...
            inputs=tf.concat([inputs_item,inputs_cate],axis=1)
    ...
    def fit(self, data,sample_store=10000000):
        ...
        for epoch in range(self.n_epochs):
            epoch_cost = []
            state = [np.zeros([self.batch_size, self.item_size+self.category_size], dtype=np.float32) for _ in range(self.layers)]
            ...
            if np.isnan(avgc):
                ...
                self.error_during_train = True
                return
            self.saver.save(self.sess, '{}/gru-model_rsc15_con_100_20_1'.format(self.checkpoint_dir), global_step=epoch)
