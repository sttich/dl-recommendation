...
class GRU4Rec:
    ...
    def __init__(self, sess, args):
        ...
        #self.rnn_size = args.behavior_size + args.item_size
        ...
        self.predict_state_item = [np.zeros([self.batch_size, self.item_size], dtype=np.float32) for _ in range(self.layers)]
        self.predict_state_cate = [np.zeros([self.batch_size, self.category_size], dtype=np.float32) for _ in range(self.layers)]
        ckpt = tf.train.get_checkpoint_state(self.checkpoint_dir)
        if ckpt and ckpt.model_checkpoint_path:
            #self.saver.restore(sess, '{}/gru-model_cate_para-{}'.format(self.checkpoint_dir, args.test_model))
            self.saver.restore(sess, '{}/gru-model_rsc15_cate_100_20-{}'.format(self.checkpoint_dir, args.test_model))
    ...
    def build_model(self):
        ...
        self.state_item = [tf.placeholder(tf.float32, [self.batch_size, self.item_size], name='rnn_state') for _ in
                      range(self.layers)]
        self.state_cate = [tf.placeholder(tf.float32, [self.batch_size, self.category_size], name='rnn_state_cate') for _ in range(self.layers)]
        self.global_step = tf.Variable(0, name='global_step', trainable=False)
        self.alpha = tf.get_variable('alpha', shape=[1], initializer=tf.constant_initializer(0.5))


        with tf.variable_scope('gru_layer_item'):
            sigma = self.sigma if self.sigma != 0 else np.sqrt(6.0 / (self.n_items + self.item_size))
            if self.init_as_normal:
                initializer = tf.random_normal_initializer(mean=0, stddev=sigma)
            else:
                initializer = tf.random_uniform_initializer(minval=-sigma, maxval=sigma)

            softmax_W = tf.get_variable('softmax_w', [self.n_items, self.item_size+self.category_size], initializer=initializer)
            ...
            cell_item = rnn_cell.GRUCell(self.item_size, activation=self.hidden_act)
            drop_cell_item = rnn_cell.DropoutWrapper(cell_item, output_keep_prob=self.dropout_p_hidden)
            stacked_cell_item = rnn_cell.MultiRNNCell([drop_cell_item] * self.layers)
            inputs_item = tf.nn.embedding_lookup(embedding_item, self.X)
            output_item, state_item = stacked_cell_item(inputs_item, tuple(self.state_item))
            self.final_state_item = state_item

        with tf.variable_scope('gru_layer_cate'):

            embedding_category = tf.get_variable('embedding_category', [self.n_category, self.category_size],
                                                 initializer=initializer)

            cell_cate = rnn_cell.GRUCell(self.category_size, activation=self.hidden_act)
            drop_cell_cate = rnn_cell.DropoutWrapper(cell_cate, output_keep_prob=self.dropout_p_hidden)
            stacked_cell_cate = rnn_cell.MultiRNNCell([drop_cell_cate] * self.layers)
            inputs_cate = tf.nn.embedding_lookup(embedding_category, self.Category)
            output_cate, state_cate = stacked_cell_cate(inputs_cate, tuple(self.state_cate))
            self.final_state_cate = state_cate



        output=tf.concat([output_item,output_cate],axis=1)
    ...
    def fit(self, data,sample_store=10000000):
        ...
        for epoch in range(self.n_epochs):
            epoch_cost = []
            state_item = [np.zeros([self.batch_size, self.item_size], dtype=np.float32) for _ in range(self.layers)]
            state_cate = [np.zeros([self.batch_size, self.category_size], dtype=np.float32) for _ in range(self.layers)]
            ...
            while not finished:
                ...
                out_category = data['category_id'].values[start]
                for i in range(minlen - 1):
                    in_idx = out_idx
                    in_category = out_category
                    out_idx = data.ItemIdx.values[start + i + 1]
                    out_category = data['category_id'].values[start + i + 1]
                    ...
                    fetches = [self.cost, self.final_state_item,self.final_state_cate, self.global_step, self.lr, self.train_op]
                    #fetches = [self.cost, self.final_state_item, self.global_step, self.lr,self.train_op]
                    feed_dict = {self.X: in_idx, self.Y: y, self.Category: in_category}
                    #feed_dict = {self.X: in_idx, self.Y: y}
                    for j in range(self.layers):
                        feed_dict[self.state_item[j]] = state_item[j]
                        feed_dict[self.state_cate[j]] = state_cate[j]

                    cost, state_item,state_cate, step, lr, _ = self.sess.run(fetches, feed_dict)
                    #cost, state_item,  step, lr, _ = self.sess.run(fetches, feed_dict)
            ...
            if np.isnan(avgc):
                ...
                self.error_during_train = True
                return
            self.saver.save(self.sess, '{}/gru-model_rsc15_cate_100_20'.format(self.checkpoint_dir), global_step=epoch)

    def predict_next_batch(self, session_ids, input_item_ids, input_category, itemidmap, batch=50):
        '''
        ...
        if len(session_change) > 0:  # change internal states with session changes
            for i in range(self.layers):
                self.predict_state_item[i][session_change] = 0.0
                self.predict_state_cate[i][session_change] = 0.0
            self.current_session = session_ids.copy()

        in_idxs = itemidmap[input_item_ids]
        fetches = [self.yhat, self.final_state_item,self.final_state_cate]
        #fetches = [self.yhat, self.final_state_item]
        feed_dict = {self.X: in_idxs, self.Category: input_category}
        #feed_dict = {self.X: in_idxs}
        for i in range(self.layers):
            feed_dict[self.state_item[i]] = self.predict_state_item[i]
            feed_dict[self.state_cate[i]] = self.predict_state_cate[i]
        preds, self.predict_state_item, self.predict_state_cate= self.sess.run(fetches, feed_dict)
