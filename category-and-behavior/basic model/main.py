...
def parseArgs():
    ...
    parser.add_argument('--train', default=1, type=int)   ##1  for train  0 for test
...
if __name__ == '__main__':
    ...
    with tf.Session(config=gpu_config) as sess:
        ...
        if args.is_training:
            ...
            res = evaluation.evaluate_sessions_batch(gru, data, valid, cut_off=5, batch_size=args.batch_size)
            print('Recall@5: {}'.format(res[0]))
            print('MRR@5: {}'.format(res[1]))
            print('NDCG@5: {}'.format(res[2]))

            res = evaluation.evaluate_sessions_batch(gru, data, valid, cut_off=10, batch_size=args.batch_size)
            print('Recall@10: {}'.format(res[0]))
            print('MRR@10: {}'.format(res[1]))
            print('NDCG@10: {}'.format(res[2]))

            res = evaluation.evaluate_sessions_batch(gru, data, valid, cut_off=20, batch_size=args.batch_size)
