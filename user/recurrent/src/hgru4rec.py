...
class HGRU4Rec:
    ...
    def __init__(self, session_layers, user_layers, n_epochs=10, batch_size=50, learning_rate=0.05, momentum=0.0,
                 ...
                 user_to_output=False, user_to_session_act='tanh', seed=42, bpreg=1.0):
        ...
        elif loss == 'top1_max' or loss == 'top1-max':
            if final_act == 'linear':
                self.final_activation = self.linear
            elif final_act == 'relu':
        ...
        elif loss == 'bpr_max' or loss == 'bpr-max':
            if final_act == 'linear':
                self.final_activation = self.linear
            elif final_act == 'relu':
                self.final_activatin = self.relu
    ...
    def bpr_max(self, yhat):  # not tested
    ...
    def top1_max(self, yhat):  # not tested
        softmax_scores = self.softmax_neg(yhat)
        y = softmax_scores * (
    ...
    def predict_next_batch(self, session_ids, input_item_ids, input_user_ids,
        ...
        if predict_for_item_ids is not None:
            ...
            result=pd.DataFrame(data=preds, index=predict_for_item_ids)
